...
// Trial lifecycle engine. Lives in lib because the Vercel Hobby plan allows
// only two cron jobs and both are taken, so this runs as a second step inside
// the daily lead-drip cron rather than on a schedule of its own.
...
export async function runTrialEmails(dry = false): Promise<TrialRunResult> {
  ...
  for (const c of candidates) {
    ...
    if (ok) {
      await supa('/email_log', {
        method: 'POST',
        body: JSON.stringify({ family_id: c.familyId, email_type: c.type, sent_to: c.ctx.email }),
      })
    }
    results.push({ email: c.ctx.email, type: c.type, reason: c.reason, sent: ok ? 'yes' : 'FAILED' })
  }
  return { due: candidates.length, results }
}

export async function previewTrialEmails(): Promise<string> {
  const candidates = await findCandidates()
  if (!candidates.length) return '<p>No trial emails are due right now.</p>'
  return candidates
    .map((c) => {
      const { subject, html } = buildTrialEmail(c.ctx, c.type)
      return `<h2 style="font:600 15px system-ui;margin:28px 0 6px">${c.type} &rarr; ${c.ctx.email} <span style="color:#888;font-weight:400">(${c.reason})</span></h2>
